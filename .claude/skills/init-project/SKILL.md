---
name: init-project
description: Initialize the AGENTS.md workflow (CLAUDE.md, AGENTS.md, memory bank) in this folder. Use when the user asks to init, set up, or bootstrap the project workflow — in an empty folder (interview mode) or an existing codebase (analysis mode).
---
...
Read `AGENTS.md` §3 (Memory Bank) before writing anything.
...
6. Optional reporting-style rule for CLAUDE.md (e.g. ASD-STE100 Simplified Technical English). If declined, delete the placeholder comment from CLAUDE.md.
...
- Every claim written to the memory bank gets a `file:line` or file-level citation, per AGENTS.md §1.
...
- If the project already has a CLAUDE.md or agent instructions, merge: existing project-specific rules survive; the workflow structure from the template wraps around them. Show the user the merged CLAUDE.md before writing it.
...
3. Create `logs/` and append the first session line per AGENTS.md §2; add `logs/` to `.gitignore`.
4. Output the startup compliance statement from AGENTS.md §1.
5. Tell the user: type `startup` at the start of every future session to activate the flow.
